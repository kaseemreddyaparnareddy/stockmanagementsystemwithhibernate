...
public class AdminController {
		...
		do {
			...
			case 6:
				...
				for (StockRequestBean investorRequest : reqInfo) {
					log.info("---------------------------------------------------");
					log.info("id -------------->" + investorRequest.getRId());
					log.info("Investor Id------>" + investorRequest.getInvestorId());
					log.info("Stock Id--------->" + investorRequest.getStockId());
					log.info("---------------------------------------------------");
				}
				break;
			...
			case 7:
				...
				log.info("Enter request id");
				int rId = 0;
				try {
					rId = scanner.nextInt();
					stockRequestBean.setStockId(rId);
				} catch (StockManagementSystemExceptions smse) {
					log.error(smse.getMessage());
				} catch (InputMismatchException e) {
					log.error("Enter valid id, should contain only digits");
				}
				stockRequestBean.setInvestorId(rId);
				log.info("-------------------------");
				try {
					boolean isAccepted = adminService.acceptStock(rId);
					if (isAccepted) {
	...
	public void adminLogin() {
		...
		try {
			AdminBean login = adminService.adminLogin(adminEmailId, adminPassword);
			if (login != null) {
				adminController();
			}
		} catch (StockManagementSystemExceptions smse) {
